...
namespace OCA\Recognize\Vendor\OkBloomer;

use OCA\Recognize\Vendor\OkBloomer\Exceptions\InvalidArgumentException;
use function count;
use function crc32;
use function round;
use function max;
...
class BloomFilter
{
     ...
     */
    public function __construct(float $maxFalsePositiveRate = 0.01, ?int $numHashes = 4, int $layerSize = 32000000)
    {
        if ($maxFalsePositiveRate < 0.0 or $maxFalsePositiveRate > 1.0) {
            throw new InvalidArgumentException('Max false positive rate' . "  must be between 0 and 1, {$maxFalsePositiveRate} given.");
        }
        if (isset($numHashes) and $numHashes < 1) {
            throw new InvalidArgumentException('Number of hashes' . " must be greater than 1, {$numHashes} given.");
        }
        if ($numHashes === null) {
            $numHashes = max(1, (int) log(1.0 / $maxFalsePositiveRate, 2));
        }
        if ($layerSize < $numHashes) {
            throw new InvalidArgumentException('Layer size must be' . " greater than {$numHashes}, {$layerSize} given.");
        }
        $sliceSize = (int) round($layerSize / $numHashes);
        if ($sliceSize > self::MAX_32_BIT_INTEGER) {
            throw new InvalidArgumentException('Layer slice size' . ' must be less than ' . self::MAX_32_BIT_INTEGER . ", {$sliceSize} given.");
        }
