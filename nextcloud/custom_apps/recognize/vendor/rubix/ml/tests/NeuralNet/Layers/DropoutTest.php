...
/**
 * @group Layers
 * @covers \OCA\Recognize\Vendor\Rubix\ML\NeuralNet\Layers\Dropout
 * @internal
 */
class DropoutTest extends TestCase
{
    ...
    public function initializeForwardBackInfer() : void
    {
        ...
        $expected = [[2.0, 5.0, -0.2], [0.2, 0.0, 6.0], [0.004, -12.0, 0.0]];
        $forward = $this->layer->forward($this->input);
        $this->assertInstanceOf(Matrix::class, $forward);
        $this->assertEquals($expected, $forward->asArray());
        $gradient = $this->layer->back($this->prevGrad, $this->optimizer)->compute();
        $expected = [[0.5, 1.4, 0.2], [1.0, 0.4, 0.02], [0.5, 0.2, 0.0]];
