...
class SVR implements Estimator, Learner
{
     ...
     */
    public function __construct(float $c = 1.0, float $epsilon = 0.1, ?Kernel $kernel = null, bool $shrinking = \true, float $tolerance = 0.001, float $cacheSize = 100.0)
    {
        ...
        if ($epsilon < 0.0) {
            throw new InvalidArgumentException('Epsilon must be' . " greater than 0, {$epsilon} given.");
        }
        ...
        $options = [svm::OPT_TYPE => svm::EPSILON_SVR, svm::OPT_C => $c, svm::OPT_P => $epsilon, svm::OPT_SHRINKING => $shrinking, svm::OPT_EPS => $tolerance, svm::OPT_CACHE_SIZE => $cacheSize];
        $options += $kernel->options();
        $svm = new svm();
        $svm->setOptions($options);
        $this->svm = $svm;
        $this->params = ['c' => $c, 'epsilon' => $epsilon, 'kernel' => $kernel, 'shrinking' => $shrinking, 'tolerance' => $tolerance, 'cache size' => $cacheSize];
    }
    /**
