...
class Adaline implements Estimator, Learner, Online, RanksFeatures, Verbose, Persistable
{
     ...
     */
    public function __construct(int $batchSize = 128, ?Optimizer $optimizer = null, float $l2Penalty = 0.0001, int $epochs = 1000, float $minChange = 0.0001, int $window = 5, ?RegressionLoss $costFn = null)
    {
        if ($batchSize < 1) {
    ...
    public function train(Dataset $dataset) : void
    {
        ...
        $this->network = new FeedForward(new Placeholder1D($dataset->numFeatures()), [new Dense(1, $this->l2Penalty, \true, new Xavier2())], new Continuous($this->costFn), $this->optimizer);
    ...
    public function featureImportances() : array
    {
        ...
        return $layer->weights()->rowAsVector(0)->abs()->asArray();
    }
    /**
