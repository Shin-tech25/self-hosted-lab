...
class HalfMoon implements Generator
{
     ...
     */
    public function __construct(float $x = 0.0, float $y = 0.0, float $scale = 1.0, float $rotation = 90.0, float $noise = 0.1)
    {
        if ($scale < 0.0) {
            throw new InvalidArgumentException('Scale must be' . " greater than 0, {$scale} given.");
        }
        if ($rotation < 0.0 or $rotation > 360.0) {
            throw new InvalidArgumentException('Rotation must be' . " between 0 and 360, {$rotation} given.");
        }
        if ($noise < 0.0) {
    ...
    {
        $r = Vector::rand($n)->multiply(\M_PI)->add(\deg2rad($this->rotation));
        $x = $r->cos()->asArray();
        $y = $r->sin()->asArray();
