...
class Blob implements Generator
{
    ...
    public static function simulate(Dataset $dataset) : self
    {
        ...
        if (count($features) !== $dataset->numFeatures()) {
            throw new InvalidArgumentException('Dataset must only contain' . ' continuous features.');
        }
        $means = $stdDevs = [];
    ...
    {
        if (empty($center)) {
            throw new InvalidArgumentException('Cannot generate samples' . ' with dimensionality less than 1.');
        }
        if (\is_array($stdDev)) {
            if (count($center) !== count($stdDev)) {
                throw new InvalidArgumentException('Number of center' . ' coordinates and standard deviations must be equal.');
            }
            foreach ($stdDev as $value) {
                if ($value < 0) {
                    throw new InvalidArgumentException('Standard deviation' . " must be greater than 0, {$value} given.");
                }
            }
            $stdDev = Vector::quick($stdDev);
        } else {
            if ($stdDev < 0) {
                throw new InvalidArgumentException('Standard deviation' . " must be greater than 0, {$stdDev} given.");
            }
        }
    ...
    {
        $d = $this->dimensions();
        $samples = Matrix::gaussian($n, $d)->multiply($this->stdDev)->add($this->center)->asArray();
