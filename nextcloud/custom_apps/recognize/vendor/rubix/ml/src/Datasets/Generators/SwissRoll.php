...
use const OCA\Recognize\Vendor\Rubix\ML\HALF_PI;
/**
 * Swiss Roll
 *
...
class SwissRoll implements Generator
{
     ...
     */
    public function __construct(float $x = 0.0, float $y = 0.0, float $z = 0.0, float $scale = 1.0, float $depth = 21.0, float $noise = 0.1)
    {
        if ($scale < 0.0) {
            throw new InvalidArgumentException('Scale must be' . " greater than 0, {$scale} given.");
        }
        if ($depth < 0) {
            throw new InvalidArgumentException('Depth must be' . " greater than 0, {$depth} given.");
        }
        if ($noise < 0.0) {
            throw new InvalidArgumentException('Noise factor cannot be less' . " than 0, {$noise} given.");
        }
    ...
    {
        $t = Vector::rand($n)->multiply(2)->add(1)->multiply(\M_PI + HALF_PI);
        ...
        $noise = Matrix::gaussian($n, 3)->multiply($this->noise);
