...
namespace OCA\Recognize\Vendor\Rubix\ML\Datasets;
...
class Labeled extends Dataset
{
    ...
    {
        return new self($samples, $labels, \true);
    }
    /**
     ...
     *
     * @param iterable<\OCA\Recognize\Vendor\Rubix\ML\Datasets\Labeled> $datasets
    ...
    public static function stack(iterable $datasets) : self
    {
        ...
        foreach ($datasets as $i => $dataset) {
            if (!$dataset instanceof Labeled) {
                throw new InvalidArgumentException('Dataset must be' . ' an instance of Labeled, ' . get_class($dataset) . ' given.');
            }
            if ($dataset->empty()) {
                continue;
            }
            if (isset($lastNumFeatures) and $dataset->numFeatures() !== $lastNumFeatures) {
                throw new InvalidArgumentException("Dataset {$i} must have" . " the same number of columns, {$lastNumFeatures}" . " expected but {$dataset->numFeatures()} given.");
            }
            $samples[] = $dataset->samples();
            $labels[] = $dataset->labels();
            $lastNumFeatures = $dataset->numFeatures();
        }
        return self::quick(\array_merge(...$samples), \array_merge(...$labels));
    }
    /**
     * @param array<mixed[]> $samples
     * @param (string|int|float)[] $labels
     ...
     * @throws InvalidArgumentException
     */
    public function __construct(array $samples = [], array $labels = [], bool $verify = \true)
    {
        if (count($samples) !== count($labels)) {
            throw new InvalidArgumentException('Number of samples' . ' and labels must be equal, ' . count($samples) . ' samples but ' . count($labels) . ' labels given.');
        }
        if ($verify and $labels) {
            $labels = \array_values($labels);
            $type = DataType::detect($labels[0]);
            if (!$type->isCategorical() and !$type->isContinuous()) {
                throw new InvalidArgumentException('Label type must be' . " categorical or continuous, {$type} given.");
            }
            foreach ($labels as $offset => $label) {
                if (DataType::detect($label) != $type) {
                    throw new InvalidArgumentException('Invalid label type' . " found at offset {$offset}, {$type} expected but " . DataType::detect($label) . ' given.');
                }
                if (is_float($label) and is_nan($label)) {
                    throw new InvalidArgumentException('Labels must not' . " contain NaN values, NaN found at offset {$offset}.");
                }
            }
        }
        $this->labels = $labels;
    ...
    public function transformLabels(callable $callback) : self
    {
        ...
        foreach ($labels as $label) {
            if (!is_string($label) and !is_numeric($label)) {
                throw new RuntimeException('Label must be a string or' . ' numeric type, ' . gettype($label) . ' found.');
            }
        }
        $this->labels = $labels;
        return $this;
    ...
    {
        if ($n < 1) {
            throw new InvalidArgumentException('The number of samples' . " cannot be less than 1, {$n} given.");
        }
    ...
    {
        return self::quick(array_slice($this->samples, $offset, $n), array_slice($this->labels, $offset, $n));
    }
    /**
    ...
    {
        return self::quick(\array_splice($this->samples, $offset, $n), \array_splice($this->labels, $offset, $n));
    }
    /**
    ...
    {
        if (!$dataset instanceof Labeled) {
            throw new InvalidArgumentException('Can only merge' . ' with another Labeled dataset.');
        }
        if (!$dataset->empty() and !$this->empty()) {
            if ($dataset->numFeatures() !== $this->numFeatures()) {
                throw new InvalidArgumentException('Datasets must have' . " the same number of columns, {$this->numFeatures()}" . " expected, but {$dataset->numFeatures()} given.");
            }
        }
        return self::quick(\array_merge($this->samples, $dataset->samples()), \array_merge($this->labels, $dataset->labels()));
    }
    /**
    ...
    public function join(Dataset $dataset) : self
    {
        if ($dataset->numSamples() !== $this->numSamples()) {
            throw new InvalidArgumentException('Datasets must have' . " the same number of rows, {$this->numSamples()}" . " expected, but {$dataset->numSamples()} given.");
        }
        $samples = [];
    ...
    public function split(float $ratio = 0.5) : array
    {
        ...
        $left = self::quick(array_slice($this->samples, 0, $n), array_slice($this->labels, 0, $n));
        $right = self::quick(array_slice($this->samples, $n), array_slice($this->labels, $n));
        return [$left, $right];
    }
    /**
    ...
    {
        if ($k < 1) {
            throw new InvalidArgumentException('Cannot create less than' . " 1 fold, {$k} given.");
        }
        ...
        $folds = [];
        while (count($folds) < $k) {
            $folds[] = self::quick(\array_splice($samples, 0, $n), \array_splice($labels, 0, $n));
        }
        return $folds;
    }
    /**
    ...
    {
        if ($k < 2) {
            throw new InvalidArgumentException('Cannot create less than' . " 2 folds, {$k} given.");
        }
        $folds = [];
        foreach ($this->stratifyByLabel() as $stratum) {
    ...
    {
        return array_map([self::class, 'quick'], array_chunk($this->samples, $n), array_chunk($this->labels, $n));
    }
    /**
    ...
    public function splitByFeature(int $column, $value) : array
    {
        ...
        return [self::quick($leftSamples, $leftLabels), self::quick($rightSamples, $rightLabels)];
    }
    /**
    ...
    {
        if ($n < 1) {
            throw new InvalidArgumentException('Cannot generate subset' . " of less than 1 sample, {$n} given.");
        }
        if ($n > $this->numSamples()) {
            throw new InvalidArgumentException('Cannot generate subset' . " of more than {$this->numSamples()}, {$n} given.");
        }
    ...
    public function randomWeightedSubsetWithReplacement(int $n, array $weights) : self
    {
        ...
        if (count($weights) !== count($this->samples)) {
            throw new InvalidArgumentException('The number of weights' . ' must be equal to the number of samples in the' . ' dataset, ' . count($this->samples) . ' needed' . ' but ' . count($weights) . ' given.');
        }
        /** @var positive-int $numLevels */
        $numLevels = (int) round(sqrt(count($weights))) ?: 1;
        $levels = array_chunk($weights, $numLevels, \true);
