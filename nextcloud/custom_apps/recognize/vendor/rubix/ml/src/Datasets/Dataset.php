...
use ArrayAccess;
use Countable;
use function OCA\Recognize\Vendor\Rubix\ML\iterator_first;
use function OCA\Recognize\Vendor\Rubix\ML\iterator_filter;
...
abstract class Dataset implements ArrayAccess, IteratorAggregate, Countable
{
     ...
     */
    public function __construct(array $samples = [], bool $verify = \true)
    {
        if ($samples and $verify) {
            ...
            foreach ($samples as $row => &$sample) {
                ...
                if (count($sample) !== $n) {
                    throw new InvalidArgumentException('Number of columns' . " must be equal for all samples, {$n} expected but " . count($sample) . " given at row offset {$row}.");
                }
                foreach ($sample as $column => $value) {
                    $type = DataType::detect($value);
                    if ($type != $types[$column]) {
                        throw new InvalidArgumentException("Column {$column}" . ' must contain values of the same data type,' . " {$types[$column]} expected but {$type} given at" . " row offset {$row}.");
                    }
                }
            }
        }
     ...
     * @return static
     */
    public static abstract function fromIterator(iterable $iterator) : self;
    /**
     * Stack a number of datasets on top of each other to form a single dataset.
     *
     * @param iterable<\OCA\Recognize\Vendor\Rubix\ML\Datasets\Dataset> $datasets
     * @return static
     */
    public static abstract function stack(iterable $datasets) : self;
    /**
    ...
    public function featureType(int $offset) : DataType
    {
        ...
        if (!isset($prototype[$offset])) {
            throw new InvalidArgumentException('Column at offset' . " {$offset} does not exist.");
        }
    ...
    public function describe() : Report
    {
        ...
        foreach ($this->types() as $offset => $type) {
            $description = ['offset' => $offset, 'type' => (string) $type];
            $values = $columns[$offset];
            switch ($type->code()) {
                case DataType::CONTINUOUS:
                    [$mean, $variance] = Stats::meanVar($values);
                    [$min, $p25, $median, $p75, $max] = Stats::quantiles($values, [0.0, 0.25, 0.5, 0.75, 1.0]);
                    $description += ['mean' => $mean, 'variance' => $variance, 'standard deviation' => \sqrt($variance), 'skewness' => Stats::skewness($values, $mean), 'kurtosis' => Stats::kurtosis($values, $mean), 'min' => $min, '25%' => $p25, 'median' => $median, '75%' => $p75, 'max' => $max, 'range' => $max - $min];
                    break;
                case DataType::CATEGORICAL:
                    ...
                    $description += ['num categories' => count($probabilities), 'probabilities' => $probabilities];
                    break;
            }
            $stats[] = $description;
        }
     ...
     * @return static
     */
    public abstract function head(int $n = 10) : self;
    /**
     ...
     * @return static
     */
    public abstract function tail(int $n = 10) : self;
    /**
     ...
     * @return static
     */
    public abstract function take(int $n = 1) : self;
    /**
     ...
     * @return static
     */
    public abstract function leave(int $n = 1) : self;
    /**
     ...
     * @return static
     */
    public abstract function slice(int $offset, int $n) : self;
    /**
     ...
     * @return static
     */
    public abstract function splice(int $offset, int $n) : self;
    /**
     ...
     * @return static
     */
    public abstract function merge(Dataset $dataset) : self;
    /**
     ...
     * @return static
     */
    public abstract function join(Dataset $dataset) : self;
    /**
     ...
     */
    public abstract function split(float $ratio = 0.5) : array;
    /**
     ...
     * @return list<self>
     */
    public abstract function fold(int $k = 10) : array;
    /**
     ...
     * @return list<self>
     */
    public abstract function batch(int $n = 50) : array;
    /**
     ...
     * @return array{self,self}
     */
    public abstract function splitByFeature(int $offset, $value) : array;
    /**
     ...
     * @return array{self,self}
     */
    public abstract function spatialSplit(array $leftCentroid, array $rightCentroid, Distance $kernel);
    /**
     ...
     * @return self
     */
    public abstract function randomSubset(int $n);
    /**
     ...
     * @return self
     */
    public abstract function randomSubsetWithReplacement(int $n);
    /**
     ...
     * @return self
     */
    public abstract function randomWeightedSubsetWithReplacement(int $n, array $weights);
    /**
