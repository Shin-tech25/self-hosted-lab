...
class Hyperplane implements Generator
{
     ...
     */
    public function __construct(array $coefficients = [1, -1], float $intercept = 0.0, float $noise = 0.1)
    {
    ...
    public function generate(int $n) : Labeled
    {
        $d = $this->dimensions();
        $y = Vector::uniform($n);
        $noise = Matrix::gaussian($n, $d)->multiply($this->noise);
        $samples = $y->add($this->intercept)->asColumnMatrix()->repeat(0, $d - 1)->multiply($this->coefficients)->add($noise)->asArray();
        $labels = $y->asArray();
