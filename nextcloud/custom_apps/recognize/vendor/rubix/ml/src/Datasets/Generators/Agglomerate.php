...
class Agglomerate implements Generator
{
     ...
     *
     * @var \OCA\Recognize\Vendor\Rubix\ML\Datasets\Generators\Generator[]
     */
    ...
    /**
     * @param \OCA\Recognize\Vendor\Rubix\ML\Datasets\Generators\Generator[] $generators
    ...
    {
        if (empty($generators)) {
            throw new InvalidArgumentException('Agglomerate must contain' . ' at least 1 Generator.');
        }
        ...
        foreach ($generators as $generator) {
            if ($generator->dimensions() !== $dimensions) {
                throw new InvalidArgumentException('Agglomerate must contain' . ' Generators that produce samples of the same' . " dimensionality, {$dimensions} expected but " . " {$generator->dimensions()} given.");
            }
        }
        if (\is_array($weights)) {
            if (count($weights) !== $k) {
                throw new InvalidArgumentException('The number of weights' . " and Generators must be equal, {$k} expected but " . count($weights) . ' given.');
            }
            foreach ($weights as $weight) {
                if ($weight < 0) {
                    throw new InvalidArgumentException('Weights must be' . " positive, {$weight} given.");
                }
            }
            $total = \array_sum($weights);
            if ($total == 0) {
                throw new InvalidArgumentException('Total weight must' . ' not be equal to 0.');
            }
            foreach ($weights as &$weight) {
                $weight /= $total;
            }
    ...
    public function generate(int $n) : Labeled
    {
        ...
        return Labeled::quick($samples ? \array_merge(...$samples) : [], $labels ? \array_merge(...$labels) : []);
    }
}
