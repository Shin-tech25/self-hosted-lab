...
class Unlabeled extends Dataset
{
    ...
    public static function stack(iterable $datasets) : self
    {
        ...
        foreach ($datasets as $i => $dataset) {
            if (!$dataset instanceof Dataset) {
                throw new InvalidArgumentException('Dataset must implement' . ' the Dataset interface.');
            }
            if ($dataset->empty()) {
                continue;
            }
            if (isset($lastNumFeatures) and $dataset->numFeatures() !== $lastNumFeatures) {
                throw new InvalidArgumentException("Dataset {$i} must have" . " the same number of features, {$lastNumFeatures}" . " expected but {$dataset->numFeatures()} given.");
            }
    ...
    public function merge(Dataset $dataset) : self
    {
        if (!$dataset->empty() and !$this->empty()) {
            if ($dataset->numFeatures() !== $this->numFeatures()) {
                throw new InvalidArgumentException('Datasets must have' . " the same dimensionality, {$this->numFeatures()}" . " expected, but {$dataset->numFeatures()} given.");
            }
        }
    ...
    {
        if ($n < 1) {
            throw new InvalidArgumentException('Cannot generate a' . " subset of less than 1 sample, {$n} given.");
        }
