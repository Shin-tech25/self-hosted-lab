...
class StepDecay implements Optimizer
{
     ...
     */
    public function __construct(float $rate = 0.01, int $losses = 100, float $decay = 0.001)
    {
        if ($rate <= 0.0) {
            throw new InvalidArgumentException('Learning rate must be' . " greater than 0, {$rate} given.");
        }
        if ($losses < 1) {
            throw new InvalidArgumentException('The number of steps per' . " floor must be greater than 0, {$losses} given.");
        }
        if ($decay < 0.0) {
            throw new InvalidArgumentException('Decay rate must be' . " positive, {$decay} given.");
        }
        $this->rate = $rate;
        $this->losses = $losses;
        $this->decay = $decay;
