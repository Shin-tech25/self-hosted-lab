...
class LeakyReLU implements ActivationFunction
{
    ...
    public function __construct(float $leakage = 0.1)
    {
        if ($leakage <= 0.0 or $leakage >= 1.0) {
            throw new InvalidArgumentException('Leakage must be between' . " 0 and 1, {$leakage} given.");
        }
    ...
    {
        return $input > 0.0 ? $input : $this->leakage * $input;
    }
    /**
     * @internal
     *
    ...
    {
        return $input > 0.0 ? 1.0 : $this->leakage;
    }
    /**
