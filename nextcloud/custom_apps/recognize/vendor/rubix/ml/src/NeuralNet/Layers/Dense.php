...
class Dense implements Hidden, Parametric
{
     ...
     */
    public function __construct(int $neurons, float $l2Penalty = 0.0, bool $bias = \true, ?Initializer $weightInitializer = null, ?Initializer $biasInitializer = null)
    {
        if ($neurons < 1) {
    ...
    {
        return "Dense (neurons: {$this->neurons}, l2 penalty: {$this->l2Penalty}," . ' bias: ' . Params::toString($this->bias) . ',' . " weight initializer: {$this->weightInitializer}," . " bias initializer: {$this->biasInitializer})";
    }
}
