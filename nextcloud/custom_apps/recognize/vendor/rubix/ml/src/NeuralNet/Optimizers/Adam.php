...
class Adam implements Optimizer, Adaptive
{
    ...
    public function __construct(float $rate = 0.001, float $momentumDecay = 0.1, float $normDecay = 0.001)
    {
        ...
        if ($momentumDecay <= 0.0 or $momentumDecay >= 1.0) {
            throw new InvalidArgumentException('Momentum decay must be' . " between 0 and 1, {$momentumDecay} given.");
        }
        if ($normDecay <= 0.0 or $normDecay >= 1.0) {
            throw new InvalidArgumentException('Norm decay must be' . " between 0 and 1, {$normDecay} given.");
        }
        $this->rate = $rate;
    ...
    public function step(Parameter $param, Tensor $gradient) : Tensor
    {
        ...
        $nHat = $gradient->square()->subtract($norm)->multiply($this->normDecay);
        $norm = $norm->add($nHat);
    ...
    {
        return "Adam (rate: {$this->rate}, momentum decay: {$this->momentumDecay}," . " norm decay: {$this->normDecay})";
    }
}
