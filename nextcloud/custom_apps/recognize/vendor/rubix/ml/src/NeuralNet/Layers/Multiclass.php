...
class Multiclass implements Output
{
    ...
    public function __construct(array $classes, ?ClassificationLoss $costFn = null)
    {
        ...
        if (count($classes) < 2) {
            throw new InvalidArgumentException('Number of classes' . ' must be greater than 1, ' . count($classes) . ' given.');
        }
    ...
    public function initialize(int $fanIn) : int
    {
        ...
        if ($fanIn !== $fanOut) {
            throw new InvalidArgumentException('Fan in must be' . " equal to fan out, {$fanOut} expected but" . " {$fanIn} given.");
        }
        return $fanOut;
    }
    /**
    ...
    public function gradient(Matrix $input, Matrix $output, Matrix $expected) : Matrix
    {
        if ($this->costFn instanceof CrossEntropy) {
            return $output->subtract($expected)->divide($output->n());
        }
        $dLoss = $this->costFn->differentiate($output, $expected)->divide($output->n());
        return $this->softmax->differentiate($input, $output)->multiply($dLoss);
    }
    /**
