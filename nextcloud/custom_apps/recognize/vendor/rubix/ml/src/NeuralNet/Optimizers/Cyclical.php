...
class Cyclical implements Optimizer
{
     ...
     */
    public function __construct(float $lower = 0.001, float $upper = 0.006, int $losses = 2000, float $decay = 0.9999400000000001)
    {
        if ($lower <= 0.0) {
            throw new InvalidArgumentException('Lower bound must be' . " greater than 0, {$lower} given.");
        }
        if ($lower > $upper) {
            throw new InvalidArgumentException('Lower bound cannot be' . ' reater than the upper bound.');
        }
        if ($losses < 1) {
            throw new InvalidArgumentException('The number of steps per' . " cycle must be greater than 0, {$losses} given.");
        }
    ...
    {
        return "Cyclical (lower: {$this->lower}, upper: {$this->upper}," . " steps: {$this->losses}, decay: {$this->decay})";
    }
}
