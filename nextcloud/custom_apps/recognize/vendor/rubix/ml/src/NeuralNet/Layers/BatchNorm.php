...
class BatchNorm implements Hidden, Parametric
{
     ...
     * @var \Tensor\Vector|null
     */
    protected ?\OCA\Recognize\Vendor\Tensor\Vector $variance = null;
    /**
     * A cache of inverse standard deviations calculated during the forward pass.
     *
     * @var \Tensor\Vector|null
     */
    protected ?\OCA\Recognize\Vendor\Tensor\Vector $stdInv = null;
    /**
    ...
    public function forward(Matrix $input) : Matrix
    {
        ...
        $this->mean = $this->mean->multiply(1.0 - $this->decay)->add($mean->multiply($this->decay));
        $this->variance = $this->variance->multiply(1.0 - $this->decay)->add($variance->multiply($this->decay));
        $this->stdInv = $stdInv;
        $this->xHat = $xHat;
        return $this->gamma->param()->multiply($xHat)->add($this->beta->param());
    }
    /**
    ...
    public function infer(Matrix $input) : Matrix
    {
        ...
        $xHat = $input->subtract($this->mean)->divide($this->variance->sqrt());
        return $this->gamma->param()->multiply($xHat)->add($this->beta->param());
    }
    /**
    ...
    public function back(Deferred $prevGradient, Optimizer $optimizer) : Deferred
    {
        ...
        return new Deferred([$this, 'gradient'], [$dOut, $gamma, $stdInv, $xHat]);
    }
    /**
    ...
    public function gradient(Matrix $dOut, ColumnVector $gamma, ColumnVector $stdInv, Matrix $xHat) : Matrix
    {
        ...
        return $dXHat->multiply($dOut->m())->subtract($dXHatSigma)->subtract($xHat->multiply($xHatSigma))->multiply($stdInv->divide($dOut->m()));
    }
    /**
    ...
    {
        return "Batch Norm (decay: {$this->decay}, beta initializer: {$this->betaInitializer}," . " gamma initializer: {$this->gammaInitializer})";
    }
}
