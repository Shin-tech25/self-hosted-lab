...
class MeanShift implements Estimator, Learner, Probabilistic, Verbose, Persistable
{
     ...
     * @return float
     */
    public static function estimateRadius(Dataset $dataset, float $percentile = 30.0, ?Distance $kernel = null) : float
    {
        if ($percentile < 0.0 or $percentile > 100.0) {
            throw new InvalidArgumentException('Percentile must be' . " between 0 and 100, {$percentile} given.");
        }
     ...
     * @throws InvalidArgumentException
     */
    public function __construct(float $radius, float $ratio = 0.1, int $epochs = 100, float $minShift = 0.0001, ?Spatial $tree = null, ?Seeder $seeder = null)
    {
        if ($radius <= 0.0) {
        ...
        if ($minShift < 0.0) {
            throw new InvalidArgumentException('Minimum shift must be' . " greater than 0, {$minShift} given.");
        }
        $this->radius = $radius;
    ...
    {
        return ['radius' => $this->radius, 'ratio' => $this->ratio, 'epochs' => $this->epochs, 'min shift' => $this->minShift, 'tree' => $this->tree, 'seeder' => $this->seeder];
    }
    /**
