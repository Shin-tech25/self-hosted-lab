...
class FuzzyCMeans implements Estimator, Learner, Probabilistic, Verbose, Persistable
{
     ...
     */
    public function __construct(int $c, float $fuzz = 2.0, int $epochs = 300, float $minChange = 0.0001, ?Distance $kernel = null, ?Seeder $seeder = null)
    {
        if ($c < 1) {
            throw new InvalidArgumentException('C must be greater' . " than 0, {$c} given.");
        }
        if ($fuzz <= 1.0) {
            throw new InvalidArgumentException('Fuzz factor must be' . " greater than 1, {$fuzz} given.");
        }
        if ($epochs < 0) {
    ...
    {
        return ['c' => $this->c, 'fuzz' => $this->fuzz, 'epochs' => $this->epochs, 'min change' => $this->minChange, 'kernel' => $this->kernel, 'seeder' => $this->seeder];
    }
    /**
