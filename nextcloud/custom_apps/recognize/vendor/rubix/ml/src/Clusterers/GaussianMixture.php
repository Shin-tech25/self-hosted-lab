...
class GaussianMixture implements Estimator, Learner, Probabilistic, Verbose, Persistable
{
     ...
     */
    public function __construct(int $k, float $smoothing = 1.0E-9, int $epochs = 100, float $minChange = 0.001, ?Seeder $seeder = null)
    {
        if ($k < 1) {
    ...
    {
        return ['k' => $this->k, 'smoothing' => $this->smoothing, 'epochs' => $this->epochs, 'min change' => $this->minChange, 'seeder' => $this->seeder];
    }
    /**
    ...
    public function train(Dataset $dataset) : void
    {
        ...
        for ($epoch = 1; $epoch <= $this->epochs; ++$epoch) {
            ...
            if ($this->logger) {
                ...
                $message = "Epoch: {$epoch}, " . "Loss: {$loss}, " . "Loss Change: {$lossDirection}{$lossChange}";
