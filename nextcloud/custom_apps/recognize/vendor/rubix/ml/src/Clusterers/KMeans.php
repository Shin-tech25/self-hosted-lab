...
class KMeans implements Estimator, Learner, Online, Probabilistic, Verbose, Persistable
{
     ...
     */
    public function __construct(int $k, int $batchSize = 128, int $epochs = 1000, float $minChange = 0.0001, int $window = 5, ?Distance $kernel = null, ?Seeder $seeder = null)
    {
        if ($k < 1) {
            throw new InvalidArgumentException('K must be greater' . " than 0, {$k} given.");
        }
        if ($batchSize < 1) {
    ...
    {
        return ['k' => $this->k, 'batch size' => $this->batchSize, 'epochs' => $this->epochs, 'min change' => $this->minChange, 'window' => $this->window, 'kernel' => $this->kernel, 'seeder' => $this->seeder];
    }
    /**
    ...
    public function partial(Dataset $dataset) : void
    {
        ...
        SpecificationChain::with([new DatasetIsNotEmpty($dataset), new SamplesAreCompatibleWithEstimator($dataset, $this), new DatasetHasDimensionality($dataset, count(\current($this->centroids)))])->check();
        if ($this->logger) {
        ...
        for ($epoch = 1; $epoch <= $this->epochs; ++$epoch) {
            ...
            if ($this->logger) {
                ...
                $message = "Epoch: {$epoch}, " . "Inertia: {$loss}, " . "Loss Change: {$lossDirection}{$lossChange}";
