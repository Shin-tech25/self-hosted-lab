...
use OCA\Recognize\Vendor\Rubix\ML\Graph\Nodes\Neighborhood;
...
class KDTree implements BinaryTree, Spatial
{
    ...
    {
        if ($maxLeafSize < 1) {
            throw new InvalidArgumentException('At least one sample is required' . " to form a neighborhood, {$maxLeafSize} given.");
        }
        if ($kernel and !in_array(DataType::continuous(), $kernel->compatibility())) {
            throw new InvalidArgumentException('Distance kernel must be' . ' compatible with continuous features.');
        }
     ...
     *
     * @param list<int|float> $sample
     * @return list<\OCA\Recognize\Vendor\Rubix\ML\Graph\Nodes\Node|null>
     */
