...
namespace OCA\Recognize\Vendor\Rubix\ML\Graph\Trees;

use OCA\Recognize\Vendor\Rubix\ML\Encoding;
use OCA\Recognize\Vendor\Rubix\ML\Datasets\Labeled;
use OCA\Recognize\Vendor\Rubix\ML\Graph\Nodes\Split;
...
use OCA\Recognize\Vendor\Rubix\ML\Graph\Nodes\HasBinaryChildren;
 ...
 *
 * @implements IteratorAggregate<int,\OCA\Recognize\Vendor\Rubix\ML\Graph\Nodes\Decision>
 * @internal
 */
abstract class DecisionTree implements BinaryTree, IteratorAggregate
{
     ...
     */
    public function __construct(int $maxHeight, int $maxLeafSize, float $minPurityIncrease)
    {
        if ($maxHeight < 1) {
            throw new InvalidArgumentException('Tree must have' . " depth greater than 0, {$maxHeight} given.");
        }
        if ($maxLeafSize < 1) {
            throw new InvalidArgumentException('At least one sample is' . " required to form a leaf node, {$maxLeafSize} given.");
        }
        if ($minPurityIncrease < 0.0) {
            throw new InvalidArgumentException('Min purity increase' . " must be greater than 0, {$minPurityIncrease} given.");
        }
     ...
     *
     * @return \Generator<\OCA\Recognize\Vendor\Rubix\ML\Graph\Nodes\BinaryNode>
     */
     ...
     * @return Split
     */
    protected abstract function split(Labeled $dataset) : Split;
    /**
     ...
     * @return Outcome
     */
    protected abstract function terminate(Labeled $dataset);
    /**
     ...
     * @return float
     */
    protected abstract function impurity(array $labels) : float;
    /**
     ...
     * @param int $depth
     */
    protected function _exportGraphviz(string &$carry, int &$nodesCounter, BinaryNode $node, ?int $maxDepth = null, ?array $featureNames = null, ?int $parentId = null, ?int $leftRight = null, int $depth = 0) : void
    {
        ++$depth;
