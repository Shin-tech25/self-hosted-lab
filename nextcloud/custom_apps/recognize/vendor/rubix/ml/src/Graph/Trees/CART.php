...
abstract class CART extends DecisionTree
{
     ...
     */
    public function __construct(int $maxHeight, int $maxLeafSize, float $minPurityIncrease, ?int $maxFeatures, ?int $maxBins)
    {
        parent::__construct($maxHeight, $maxLeafSize, $minPurityIncrease);
        if (isset($maxFeatures) and $maxFeatures < 1) {
            throw new InvalidArgumentException('Tree must consider at least 1' . " feature to determine a split, {$maxFeatures} given.");
        }
        if (isset($maxBins) and $maxBins < 2) {
            throw new InvalidArgumentException('At least two bins are' . " required to determine a split, {$maxBins} given.");
        }
    ...
    protected function split(Labeled $dataset) : Split
    {
        ...
        return new Split($bestColumn, $bestValue, $bestSubsets, $bestImpurity, $m);
    }
}
