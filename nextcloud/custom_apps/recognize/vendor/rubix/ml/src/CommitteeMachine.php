...
class CommitteeMachine implements Estimator, Learner, Parallel, Persistable
{
     ...
     * @var list<int>
     */
    protected const COMPATIBLE_ESTIMATOR_TYPES = [EstimatorType::CLASSIFIER, EstimatorType::REGRESSOR, EstimatorType::ANOMALY_DETECTOR];
    /**
     * The committee of experts. i.e. the ensemble of estimators.
     *
     * @var list<\OCA\Recognize\Vendor\Rubix\ML\Learner>
     */
     ...
     *
     * @var list<\OCA\Recognize\Vendor\Rubix\ML\DataType>
     */
    ...
    /**
     * @param \OCA\Recognize\Vendor\Rubix\ML\Learner[] $experts
    ...
    public function __construct(array $experts, ?array $influences = null)
    {
        ...
        foreach ($experts as $expert) {
            ...
            if (!in_array($expert->type()->code(), self::COMPATIBLE_ESTIMATOR_TYPES)) {
                throw new InvalidArgumentException('Committee only supports' . ' classifiers, regressors, and anomaly detectors, ' . " {$expert->type()} given.");
            }
            if ($expert->type() != $prototype->type()) {
                throw new InvalidArgumentException('Experts must be of' . " the same type, {$prototype->type()} expected but" . " {$expert->type()} given.");
            }
            $compatibilities[] = $expert->compatibility();
        }
        $compatibility = \array_values(\array_intersect(...$compatibilities));
        if (count($compatibility) < 1) {
            throw new InvalidArgumentException('Committee must have at' . ' least 1 compatible data type in common.');
        }
        $k = count($experts);
        if ($influences) {
            if (count($influences) !== $k) {
                throw new InvalidArgumentException('Number of influences' . " must be equal to the number of experts, {$k}" . ' expected but ' . count($influences) . ' given.');
            }
            $total = \array_sum($influences);
            if ($total <= 0) {
                throw new InvalidArgumentException('Total influence must' . " be greater than 0, {$total} given.");
            }
    ...
    {
        return ['experts' => $this->experts, 'influences' => $this->influences];
    }
    /**
     ...
     *
     * @return list<\OCA\Recognize\Vendor\Rubix\ML\Learner>
     */
    ...
    {
        $specifications = [new DatasetIsNotEmpty($dataset), new SamplesAreCompatibleWithEstimator($dataset, $this)];
