...
use OCA\Recognize\Vendor\Rubix\ML\Exceptions\ClassRevisionMismatch;
...
use const OCA\Recognize\Vendor\Rubix\ML\VERSION as LIBRARY_VERSION;
/**
 * RBX
 *
...
class RBX implements Serializer
{
     ...
     * @var string
     */
    protected const IDENTIFIER_STRING = "\xa1RBX\r\n\x1a\n";
    /**
    ...
    public function serialize(Persistable $persistable) : Encoding
    {
        ...
        $header = JSON::encode(['library' => ['version' => LIBRARY_VERSION], 'class' => ['name' => get_class($persistable), 'revision' => $persistable->revision()], 'data' => ['checksum' => ['type' => self::CHECKSUM_HASH_TYPE, 'hash' => $hash], 'length' => $encoding->bytes()]]);
