...
class BM25Transformer implements Transformer, Stateful, Elastic
{
    ...
    {
        if ($dampening < 0.0) {
            throw new InvalidArgumentException('Dampening must be greater' . " than 0, {$dampening} given.");
        }
        if ($normalization < 0.0 or $normalization > 1.0) {
            throw new InvalidArgumentException('Normalization must be between' . " 0 and 1, {$normalization} given.");
        }
     ...
     *
     * @return \OCA\Recognize\Vendor\Rubix\ML\DataType[]
     */
