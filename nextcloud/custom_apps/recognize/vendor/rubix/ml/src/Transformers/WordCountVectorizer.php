...
class WordCountVectorizer implements Transformer, Stateful, Persistable
{
     ...
     */
    public function __construct(int $maxVocabularySize = \PHP_INT_MAX, int $minDocumentCount = 1, float $maxDocumentRatio = 0.8, ?Tokenizer $tokenizer = null)
    {
        if ($maxVocabularySize < 1) {
            throw new InvalidArgumentException('Max vocabulary size must be' . " greater than 0, {$maxVocabularySize} given.");
        }
        if ($minDocumentCount <= 0) {
            throw new InvalidArgumentException('Min document count' . " must be greater than 0, {$minDocumentCount} given.");
        }
        if ($maxDocumentRatio < 0.0 or $maxDocumentRatio > 1.0) {
            throw new InvalidArgumentException('Max document ratio' . " must be between 0 and 1, {$maxDocumentRatio} given.");
        }
    ...
    public function fit(Dataset $dataset) : void
    {
        ...
        foreach ($dataset->featureTypes() as $column => $type) {
            if ($type->isCategorical()) {
                ...
                if (empty($tfs)) {
                    throw new RuntimeException('Cannot create vocabulary' . ' from corpus with given document frequency' . " constraints on column {$column}.");
                }
                if (count($tfs) > $this->maxVocabularySize) {
                    arsort($tfs);
                    $tfs = array_slice($tfs, 0, $this->maxVocabularySize, \true);
                }
    ...
    {
        return "Word Count Vectorizer (max vocabulary size: {$this->maxVocabularySize}," . " min document count: {$this->minDocumentCount}," . " max document ratio: {$this->maxDocumentRatio}," . " tokenizer: {$this->tokenizer})";
    }
}
