...
class ImageRotator implements Transformer
{
    ...
    public function __construct(float $offset, float $jitter = 0.0)
    {
        ...
        if ($offset < -360.0 or $offset > 360.0) {
            throw new InvalidArgumentException('Offset must be ' . " greater than -360, and less than 360 and {$offset} given.");
        }
        if ($jitter < 0.0 or $jitter > 1.0) {
            throw new InvalidArgumentException('Jitter must be ' . " greater than 0, and less than 1 and {$jitter} given.");
        }
        $this->offset = $offset;
        $this->jitter = $jitter;
    }
            ...
            if (DataType::detect($value)->isImage()) {
                ...
                if ($rotated) {
                    ...
                    if ($originalHeight !== $newHeight or $originalWidth !== $newWidth) {
                        $rotated = \imagecrop($rotated, ['x' => $newWidth / 2 - $originalWidth / 2, 'y' => $newHeight / 2 - $originalHeight / 2, 'width' => $originalWidth, 'height' => $originalHeight]);
                    }
                    $value = $rotated;
                }
            }
