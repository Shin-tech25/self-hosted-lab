...
class TSNE implements Transformer, Verbose
{
     ...
     * @var float
     */
    protected const PERPLEXITY_TOLERANCE = 1.0E-5;
    /**
     * The scaling coefficient of the initial embedding.
     *
     * @var float
     */
    protected const Y_INIT_SCALE = 0.0001;
    /**
     ...
     */
    public function __construct(int $dimensions = 2, float $rate = 100.0, int $perplexity = 30, float $exaggeration = 12.0, int $epochs = 1000, float $minGradient = 1.0E-7, int $window = 5, ?Distance $kernel = null)
    {
        if ($dimensions < 1) {
        ...
        if ($perplexity < 1) {
            throw new InvalidArgumentException('Perplexity must be' . " greater than 0, {$perplexity} given.");
        }
        if ($exaggeration < 1.0) {
            throw new InvalidArgumentException('Exaggeration must be' . " greater than 1, {$exaggeration} given.");
        }
        if ($epochs < 1) {
            throw new InvalidArgumentException('Number of epochs' . " must be greater than 0, {$epochs} given.");
        }
        if ($minGradient < 0.0) {
            throw new InvalidArgumentException('Minimum gradient must be' . " greater than 0, {$minGradient} given.");
        }
        if ($window < 1) {
    ...
    public function transform(array &$samples) : void
    {
        ...
        $p = Matrix::quick($this->affinities($distances))->multiply($this->exaggeration);
        $y = Matrix::gaussian($m, $this->dimensions)->multiply(self::Y_INIT_SCALE);
        ...
        for ($epoch = 1; $epoch <= $this->epochs; ++$epoch) {
            ...
            $velocity = $velocity->multiply($momentum)->subtract($gradient->multiply($this->rate));
            $y = $y->add($velocity);
    ...
    {
        $q = $distances->divide($this->dofs)->add(1.0)->pow((1.0 + $this->dofs) / -2.0);
        ...
        return Matrix::quick($gradient)->multiply($this->c);
    }
    /**
    ...
    {
        $value = $direction < 0.0 ? $gain + self::GAIN_ACCELERATE : $gain * self::GAIN_BRAKE;
    ...
    {
        return 't-SNE (' . Params::stringify(['dimensions' => $this->dimensions, 'rate' => $this->rate, 'perplexity' => $this->perplexity, 'exaggeration' => $this->exaggeration, 'epochs' => $this->epochs, 'min gradient' => $this->minGradient, 'window' => $this->window, 'kernel' => $this->kernel]) . ')';
    }
}
