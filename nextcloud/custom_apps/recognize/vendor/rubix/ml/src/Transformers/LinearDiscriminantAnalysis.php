...
class LinearDiscriminantAnalysis implements Transformer, Stateful, Persistable
{
    ...
    public function fit(Dataset $dataset) : void
    {
        if (!$dataset instanceof Labeled) {
            throw new InvalidArgumentException('Transformer requires a' . ' Labeled training set.');
        }
        SamplesAreCompatibleWithTransformer::with($dataset, $this)->check();
        if ($dataset->labelType() != DataType::categorical()) {
            throw new InvalidArgumentException('Transformer requires' . " categorical labels, {$dataset->labelType()} given.");
        }
        [$m, $n] = $dataset->shape();
        $sW = Matrix::zeros($n, $n);
        foreach ($dataset->stratifyByLabel() as $stratum) {
            $prior = $stratum->numSamples() / $m;
            $sW = Matrix::quick($stratum->samples())->transpose()->covariance()->multiply($prior)->add($sW);
        }
        $eig = Matrix::quick($dataset->samples())->transpose()->covariance()->subtract($sW)->eig(\true);
    ...
    public function transform(array &$samples) : void
    {
        ...
        $samples = Matrix::build($samples)->matmul($this->eigenvectors)->asArray();
    }
    /**
