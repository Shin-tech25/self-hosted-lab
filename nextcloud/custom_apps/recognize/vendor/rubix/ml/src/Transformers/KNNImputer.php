...
class KNNImputer implements Transformer, Stateful, Persistable
{
     ...
     *
     * @var \OCA\Recognize\Vendor\Rubix\ML\DataType[]|null
     */
     ...
     */
    public function __construct(int $k = 5, bool $weighted = \false, string $categoricalPlaceholder = '?', ?Spatial $tree = null)
    {
        if ($k < 1) {
            throw new InvalidArgumentException('At least 1 donor is required' . " to impute a value, {$k} given.");
        }
        if ($tree and in_array(DataType::continuous(), $tree->kernel()->compatibility())) {
            $kernel = $tree->kernel();
            if (!$kernel instanceof NaNSafe) {
                throw new InvalidArgumentException('Continuous distance kernels' . ' must implement the NaNSafe interface.');
            }
        }
    ...
    {
        return "KNN Imputer (k: {$this->k}, weighted: {$this->weighted}," . " categorical placeholder: {$this->categoricalPlaceholder}," . " tree: {$this->tree})";
    }
}
