...
class TfIdfTransformer implements Transformer, Stateful, Elastic, Reversible, Persistable
{
    ...
    {
        SpecificationChain::with([new DatasetIsNotEmpty($dataset), new SamplesAreCompatibleWithTransformer($dataset, $this)])->check();
        if ($this->dfs === null) {
            $this->fit($dataset);
            return;
        }
    ...
    {
        return "TF-IDF Transformer (smoothing: {$this->smoothing}, dampening: " . Params::toString($this->dampening) . ')';
    }
}
