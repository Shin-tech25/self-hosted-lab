...
class GaussianMLE implements Estimator, Learner, Online, Scoring, Persistable
{
     ...
     */
    public function __construct(float $contamination = 0.1, float $smoothing = 1.0E-9)
    {
    ...
    {
        return ['contamination' => $this->contamination, 'smoothing' => $this->smoothing];
    }
    /**
    ...
    public function partial(Dataset $dataset) : void
    {
        ...
        SpecificationChain::with([new DatasetIsNotEmpty($dataset), new SamplesAreCompatibleWithEstimator($dataset, $this), new DatasetHasDimensionality($dataset, \count($this->means))])->check();
        ...
        foreach ($dataset->features() as $column => $values) {
            ...
            $this->variances[$column] = ($this->n * $oldVariance + $n * $variance + $this->n / ($n * $weight) * ($n * $oldMean - $n * $mean) ** 2) / $weight;
        }
        $epsilon = \max($this->smoothing * \max($this->variances), CPU::epsilon());
        foreach ($this->variances as &$variance) {
            $variance += $epsilon;
