...
class OneClassSVM implements Estimator, Learner
{
     ...
     */
    public function __construct(float $nu = 0.5, ?Kernel $kernel = null, bool $shrinking = \true, float $tolerance = 0.001, float $cacheSize = 100.0)
    {
        SpecificationChain::with([new ExtensionIsLoaded('svm'), new ExtensionMinimumVersion('svm', '0.2.0')])->check();
        if ($nu < 0.0 or $nu > 1.0) {
            throw new InvalidArgumentException('Nu must be between' . "0 and 1, {$nu} given.");
        }
        ...
        $options = [svm::OPT_TYPE => svm::ONE_CLASS, svm::OPT_NU => $nu, svm::OPT_SHRINKING => $shrinking, svm::OPT_EPS => $tolerance, svm::OPT_CACHE_SIZE => $cacheSize];
        $options += $kernel->options();
        $svm = new svm();
        $svm->setOptions($options);
        $this->svm = $svm;
        $this->params = ['nu' => $nu, 'kernel' => $kernel, 'shrinking' => $shrinking, 'tolerance' => $tolerance, 'cache size' => $cacheSize];
    }
    /**
