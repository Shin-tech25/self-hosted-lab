...
class RobustZScore implements Estimator, Learner, Scoring, Persistable
{
     ...
     */
    public function __construct(float $threshold = 3.5, float $beta = 0.5, float $smoothing = 1.0E-9)
    {
        if ($threshold <= 0.0) {
            throw new InvalidArgumentException('Threshold must be' . " greater than 0, {$threshold} given.");
        }
        if ($beta < 0.0 or $beta > 1.0) {
            throw new InvalidArgumentException('Beta must be' . " between 0 and 1, {$beta} given.");
        }
        if ($smoothing <= 0.0) {
    ...
    {
        return ['threshold' => $this->threshold, 'beta' => $this->beta, 'smoothing' => $this->smoothing];
    }
    /**
    ...
    protected function zHat(array $sample) : float
    {
        ...
        foreach ($sample as $column => $value) {
            $scores[] = abs(self::ETA * ($value - $this->medians[$column]) / $this->mads[$column]);
        }
        $zHat = (1.0 - $this->beta) * Stats::mean($scores) + $this->beta * max($scores);
        return $zHat;
    }
    /**
