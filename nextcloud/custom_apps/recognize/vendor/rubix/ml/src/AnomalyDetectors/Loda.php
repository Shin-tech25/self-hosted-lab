...
use function log;
use function sqrt;
use const OCA\Recognize\Vendor\Rubix\ML\LOG_EPSILON;
/**
 * Loda
 *
...
class Loda implements Estimator, Learner, Online, Scoring, Persistable
{
    ...
    public function __construct(float $contamination = 0.1, int $estimators = 100, ?int $bins = null)
    {
        ...
        if (isset($bins) and $bins < self::MIN_BINS) {
            throw new InvalidArgumentException('Bins must be greater' . ' than ' . self::MIN_BINS . ", {$bins} given.");
        }
    ...
    {
        return ['contamination' => $this->contamination, 'estimators' => $this->estimators, 'bins' => $this->bins];
    }
    /**
    ...
    public function train(Dataset $dataset) : void
    {
        ...
        if ($n >= self::MIN_SPARSE_DIMENSIONS) {
            $mask = Matrix::rand($n, $this->estimators)->less(sqrt($n) / $n);
            $this->r = $this->r->multiply($mask);
        }
        $projections = Matrix::quick($dataset->samples())->matmul($this->r)->transpose()->asArray();
    ...
    public function partial(Dataset $dataset) : void
    {
        ...
        SpecificationChain::with([new DatasetIsNotEmpty($dataset), new SamplesAreCompatibleWithEstimator($dataset, $this), new DatasetHasDimensionality($dataset, $this->r->m())])->check();
        $projections = Matrix::quick($dataset->samples())->matmul($this->r)->transpose()->asArray();
        ...
        foreach ($projections as $i => $values) {
            ...
            foreach ($values as $j => $value) {
                foreach ($edges as $k => $edge) {
                    if ($value <= $edge) {
                        $count = $counts[$k];
                        $densities[$j] += $count > 0 ? -log($count / $this->n) : -LOG_EPSILON;
                        break;
                    }
                }
            }
