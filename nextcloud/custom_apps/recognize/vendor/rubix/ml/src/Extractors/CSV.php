...
class CSV implements Extractor, Exporter
{
     ...
     */
    public function __construct(string $path, bool $header = \false, string $delimiter = ',', string $enclosure = '"', string $escape = '\\')
    {
        if (empty($path)) {
        ...
        if (strlen($delimiter) !== 1) {
            throw new InvalidArgumentException('Delimiter must be' . ' a single character, ' . strlen($delimiter) . ' given.');
        }
        if (strlen($enclosure) !== 1) {
            throw new InvalidArgumentException('Enclosure must be' . ' a single character, ' . strlen($enclosure) . ' given.');
        }
        if (strlen($escape) !== 1) {
            throw new InvalidArgumentException('Escape character must be' . ' a single character, ' . strlen($escape) . ' given.');
        }
        $this->path = $path;
        $this->header = $header;
