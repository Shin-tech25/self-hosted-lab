...
class AdaBoost implements Estimator, Learner, Probabilistic, Verbose, Persistable
{
     ...
     *
     * @var \OCA\Recognize\Vendor\Rubix\ML\Learner[]|null
     */
     ...
     */
    public function __construct(?Learner $base = null, float $rate = 1.0, float $ratio = 0.8, int $epochs = 100, float $minChange = 0.0001, int $window = 5)
    {
        if ($base and !$base->type()->isClassifier()) {
            throw new InvalidArgumentException('Base Estimator must be' . " a classifier, {$base->type()} given.");
        }
        if ($rate < 0.0) {
    ...
    {
        return ['base' => $this->base, 'rate' => $this->rate, 'ratio' => $this->ratio, 'epochs' => $this->epochs, 'min change' => $this->minChange, 'window' => $this->window];
    }
    /**
    ...
    public function train(Dataset $dataset) : void
    {
        ...
        for ($epoch = 1; $epoch <= $this->epochs; ++$epoch) {
            ...
            if ($this->logger) {
                ...
                $message = "Epoch: {$epoch}, " . "Exponential Loss: {$loss}, " . "Loss Change: {$lossDirection}{$lossChange}";
            ...
            $influence = $this->rate * (log((1.0 - $loss) / ($loss ?: EPSILON)) + log($k - 1));
    ...
    {
        return \array_map('OCA\\Recognize\\Vendor\\Rubix\\ML\\argmax', $this->score($dataset));
    }
    /**
