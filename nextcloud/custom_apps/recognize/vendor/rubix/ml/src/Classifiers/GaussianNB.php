...
class GaussianNB implements Estimator, Learner, Online, Probabilistic, Persistable
{
     ...
     */
    public function __construct(?array $priors = null, float $smoothing = 1.0E-9)
    {
        $logPriors = [];
        if ($priors) {
            $total = \array_sum($priors);
            if ($total == 0) {
                throw new InvalidArgumentException('Total class prior' . ' probability cannot be equal to 0.');
            }
            foreach ($priors as $class => $prior) {
                if ($prior < 0) {
                    throw new InvalidArgumentException('Prior probability' . " must be greater than 0, {$prior} given.");
                }
    ...
    {
        return ['priors' => $this->fitPriors ? null : $this->priors(), 'smoothing' => $this->smoothing];
    }
    /**
        ...
        foreach ($dataset->stratifyByLabel() as $class => $stratum) {
            if (isset($this->means[$class])) {
                ...
                foreach ($stratum->features() as $column => $values) {
                    ...
                    $means[] = ($n * $mean + $oldWeight * $oldMean) / $weight;
                    $variances[] = ($oldWeight * $oldVariance + $n * $variance + $oldWeight / ($n * $weight) * ($n * $oldMean - $n * $mean) ** 2) / $weight;
                }
            } else {
                $means = $variances = [];
