...
use OCA\Recognize\Vendor\Rubix\ML\Regressors\ExtraTreeRegressor;
...
class LogitBoost implements Estimator, Learner, Probabilistic, RanksFeatures, Verbose, Persistable
{
     ...
     * @var class-string[]
     */
    public const COMPATIBLE_BOOSTERS = [RegressionTree::class, ExtraTreeRegressor::class];
    /**
     ...
     */
    public function __construct(?Learner $booster = null, float $rate = 0.1, float $ratio = 0.5, int $epochs = 1000, float $minChange = 0.0001, int $window = 5, float $holdOut = 0.1, ?Metric $metric = null)
    {
        if ($booster and !in_array(get_class($booster), self::COMPATIBLE_BOOSTERS)) {
            throw new InvalidArgumentException('Booster is not compatible' . ' with the ensemble.');
        }
        if ($rate <= 0.0) {
    ...
    {
        return ['booster' => $this->booster, 'rate' => $this->rate, 'ratio' => $this->ratio, 'epochs' => $this->epochs, 'min change' => $this->minChange, 'window' => $this->window, 'hold out' => $this->holdOut, 'metric' => $this->metric];
    }
    /**
    ...
    {
        return $this->boosters and $this->classes and $this->featureCount;
    }
    /**
    ...
    public function train(Dataset $dataset) : void
    {
        ...
        if (count($classes) !== 2) {
            throw new InvalidArgumentException('Number of classes' . ' must be exactly 2, ' . count($classes) . ' given.');
        }
        if ($this->logger) {
        ...
        for ($epoch = 1; $epoch <= $this->epochs; ++$epoch) {
            ...
            if ($this->logger) {
                ...
                $message = "Epoch: {$epoch}, " . "Cross Entropy: {$loss}, " . "Loss Change: {$lossDirection}{$lossChange}, " . "{$this->metric}: " . ($score ?? 'N/A');
            ...
            $out = array_map('OCA\\Recognize\\Vendor\\Rubix\\ML\\sigmoid', $z);
            if (isset($zTest)) {
    ...
    public function proba(Dataset $dataset) : array
    {
        ...
        foreach ($out as $probability) {
            $probabilities[] = [$classA => 1.0 - $probability, $classB => $probability];
        }
