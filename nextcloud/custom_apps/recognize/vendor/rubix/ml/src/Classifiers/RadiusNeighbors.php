...
class RadiusNeighbors implements Estimator, Learner, Probabilistic, Persistable
{
     ...
     */
    public function __construct(float $radius = 1.0, bool $weighted = \false, string $outlierClass = '?', ?Spatial $tree = null)
    {
        if ($radius <= 0.0) {
            throw new InvalidArgumentException('Radius must be' . " greater than 0, {$radius} given.");
        }
        if (empty($outlierClass)) {
            throw new InvalidArgumentException('Anomaly class' . ' cannot be an empty string.');
        }
        $this->radius = $radius;
    ...
    {
        return ['radius' => $this->radius, 'weighted' => $this->weighted, 'outlier class' => $this->outlierClass, 'tree' => $this->tree];
    }
    /**
    ...
    public function train(Dataset $dataset) : void
    {
        ...
        if (in_array($this->outlierClass, $classes)) {
            throw new RuntimeException('Training set cannot contain' . ' labels of the outlier class.');
        }
