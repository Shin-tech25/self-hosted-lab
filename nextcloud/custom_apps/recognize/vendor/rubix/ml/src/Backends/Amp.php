...
use OCA\Recognize\Vendor\Amp\Parallel\Worker\DefaultPool;
use OCA\Recognize\Vendor\Amp\Parallel\Worker\CallableTask;
use OCA\Recognize\Vendor\Amp\Parallel\Worker\Task as AmpTask;
use OCA\Recognize\Vendor\Rubix\ML\Exceptions\InvalidArgumentException;
use Generator;
use function OCA\Recognize\Vendor\Amp\call;
use function OCA\Recognize\Vendor\Amp\Promise\all;
/**
 * Amp
 *
...
class Amp implements Backend
{
     ...
     */
    protected \OCA\Recognize\Vendor\Amp\Parallel\Worker\Pool $pool;
    /**
    ...
    public function __construct(?int $workers = null)
    {
        if (isset($workers) and $workers < 1) {
            throw new InvalidArgumentException('Number of workers' . " must be greater than 0, {$workers} given.");
        }
