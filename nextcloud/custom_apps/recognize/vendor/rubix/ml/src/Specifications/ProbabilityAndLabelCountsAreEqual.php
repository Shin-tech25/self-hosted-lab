...
class ProbabilityAndLabelCountsAreEqual
{
    ...
    public function check() : void
    {
        if (\count($this->probabilities) !== \count($this->labels)) {
            throw new InvalidArgumentException('Number of probabilities and labels must be equal ' . \count($this->probabilities) . ' predictions but ' . \count($this->labels) . ' labels given.');
        }
    }
}
