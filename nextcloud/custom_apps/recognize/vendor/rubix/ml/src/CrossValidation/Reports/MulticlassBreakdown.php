...
class MulticlassBreakdown implements ReportGenerator
{
    ...
    public function generate(array $predictions, array $labels) : Report
    {
        ...
        $averages = array_fill_keys(['accuracy', 'balanced accuracy', 'f1 score', 'precision', 'recall', 'specificity', 'negative predictive value', 'false discovery rate', 'miss rate', 'fall out', 'false omission rate', 'mcc', 'informedness', 'markedness'], 0.0);
        $counts = array_fill_keys(['true positives', 'true negatives', 'false positives', 'false negatives'], 0);
        ...
        foreach ($truePos as $label => $tp) {
            ...
            $f1score = 2.0 * ($precision * $recall / ($precision + $recall ?: EPSILON));
            $mcc = ($tp * $tn - $fp * $fn) / (\sqrt(($tp + $fp) * ($tp + $fn) * ($tn + $fp) * ($tn + $fn)) ?: EPSILON);
            $cardinality = $tp + $fn;
            $table[$label] = ['accuracy' => $accuracy, 'balanced accuracy' => ($recall + $specificity) / 2.0, 'f1 score' => $f1score, 'precision' => $precision, 'recall' => $recall, 'specificity' => $specificity, 'negative predictive value' => $npv, 'false discovery rate' => 1.0 - $precision, 'miss rate' => 1.0 - $recall, 'fall out' => 1.0 - $specificity, 'false omission rate' => 1.0 - $npv, 'informedness' => $recall + $specificity - 1.0, 'markedness' => $precision + $npv - 1.0, 'mcc' => $mcc, 'true positives' => $tp, 'true negatives' => $tn, 'false positives' => $fp, 'false negatives' => $fn, 'cardinality' => $cardinality, 'proportion' => $cardinality / $n];
        ...
        return new Report(['overall' => $overall, 'classes' => $table]);
    }
}
