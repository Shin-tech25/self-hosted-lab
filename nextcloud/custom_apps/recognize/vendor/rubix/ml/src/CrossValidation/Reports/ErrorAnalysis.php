...
class ErrorAnalysis implements ReportGenerator
{
    ...
    public function generate(array $predictions, array $labels) : Report
    {
        ...
        $quantiles = Stats::quantiles($errors, [0.0, 0.25, 0.5, 0.75, 1.0]);
        return new Report(['mean absolute error' => Stats::mean($l1), 'median absolute error' => Stats::median($l1), 'mean squared error' => $mse, 'mean squared log error' => Stats::mean($sle), 'mean absolute percentage error' => 100.0 * Stats::mean($are), 'rms error' => \sqrt($mse), 'r squared' => 1.0 - $sse / ($sst ?: EPSILON), 'error mean' => $mean, 'error standard deviation' => \sqrt($variance), 'error skewness' => Stats::skewness($errors, $mean), 'error kurtosis' => Stats::kurtosis($errors, $mean), 'error min' => $quantiles[0], 'error 25%' => $quantiles[1], 'error median' => $quantiles[2], 'error 75%' => $quantiles[3], 'error max' => $quantiles[4], 'cardinality' => count($predictions)]);
    }
}
