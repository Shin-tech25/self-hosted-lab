...
class MonteCarlo implements Validator, Parallel
{
    ...
    {
        if ($simulations < 1) {
            throw new InvalidArgumentException('Number of simulations' . " must be greater than 0, {$simulations} given.");
        }
    ...
    public function test(Learner $estimator, Labeled $dataset, Metric $metric) : float
    {
        ...
        for ($i = 0; $i < $this->simulations; ++$i) {
            $dataset->randomize();
            [$testing, $training] = $stratify ? $dataset->stratifiedSplit($this->ratio) : $dataset->split($this->ratio);
