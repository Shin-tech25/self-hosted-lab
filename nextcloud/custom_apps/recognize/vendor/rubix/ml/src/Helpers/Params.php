...
class Params
{
    ...
    {
        if ($max <= $min) {
            throw new InvalidArgumentException('Maximum cannot be' . ' less than or equal to minimum.');
        }
        if ($n < 1) {
            throw new InvalidArgumentException('Cannot generate less' . ' than 1 parameter.');
        }
        if ($n > $max - $min + 1) {
            throw new InvalidArgumentException('Cannot generate more' . ' unique integers than within range of.');
        }
        $dist = [];
        while (count($dist) < $n) {
            $r = \rand($min, $max);
    ...
    public static function grid(float $min, float $max, int $n = 10) : array
    {
        ...
        if ($n < 2) {
            throw new InvalidArgumentException('Cannot generate less' . ' than 2 parameters.');
        }
