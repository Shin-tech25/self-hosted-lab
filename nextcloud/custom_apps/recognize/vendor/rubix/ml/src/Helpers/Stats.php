...
class Stats
{
    ...
    public static function weightedMean(array $values, array $weights) : float
    {
        ...
        if (count($values) !== count($weights)) {
            throw new InvalidArgumentException('The number of weights must' . ' equal the number of values.');
        }
    ...
    public static function quantiles(array $values, array $qs) : array
    {
        ...
        foreach ($qs as $q) {
            if ($q < 0.0 or $q > 1.0) {
                throw new InvalidArgumentException('Quantile must be' . " between 0 and 1, {$q} given.");
            }
            $x = $q * ($n - 1) + 1;
            $xHat = (int) $x;
            $remainder = $x - $xHat;
    ...
    {
        if (empty($values)) {
            throw new InvalidArgumentException('Median absolute deviation' . ' is undefined for empty set.');
        }
