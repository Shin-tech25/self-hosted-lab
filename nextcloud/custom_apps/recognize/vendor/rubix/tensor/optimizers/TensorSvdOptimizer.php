...
class TensorSvdOptimizer extends OptimizerAbstract
{
    ...
    public function optimize(array $expression, Call $call, CompilationContext $context)
    {
        ...
        if (\count($expression['parameters']) !== 1) {
            throw new CompilerException('SVD accepts exactly one argument, ' . \count($expression['parameters']) . 'given.', $expression);
        }
        ...
        $context->codePrinter->output("tensor_svd({$symbol}, {$resolvedParams[0]});");
