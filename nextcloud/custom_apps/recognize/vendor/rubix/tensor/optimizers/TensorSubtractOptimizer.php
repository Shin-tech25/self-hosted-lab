...
class TensorSubtractOptimizer extends OptimizerAbstract
{
    ...
    public function optimize(array $expression, Call $call, CompilationContext $context)
    {
        ...
        if (\count($expression['parameters']) !== 2) {
            throw new CompilerException('Tensor subtract accepts exactly two arguments, ' . \count($expression['parameters']) . 'given.', $expression);
        }
        ...
        $context->codePrinter->output("tensor_subtract({$symbol}, {$resolvedParams[0]}, {$resolvedParams[1]});");
