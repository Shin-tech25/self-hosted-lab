...
class TensorSetNumThreadsOptimizer extends OptimizerAbstract
{
    ...
    public function optimize(array $expression, Call $call, CompilationContext $context)
    {
        ...
        if (\count($expression['parameters']) !== 1) {
            throw new CompilerException('Set num threads accepts exactly one argument, ' . \count($expression['parameters']) . 'given.', $expression);
        }
        ...
        $context->codePrinter->output("tensor_set_num_threads({$symbol}, {$resolvedParams[0]});");
