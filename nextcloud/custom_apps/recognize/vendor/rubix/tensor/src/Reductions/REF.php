...
namespace OCA\Recognize\Vendor\Tensor\Reductions;
...
class REF
{
    ...
    public static function gaussianElimination(Matrix $a) : self
    {
        ...
        for ($i = 0; $i < $minDim; ++$i) {
            ...
            if ($b[$index][$i] == 0) {
                throw new RuntimeException('Cannot compute row echelon' . ' form of a singular matrix.');
            }
            if ($i !== $index) {
                $temp = $b[$i];
                $b[$i] = $b[$index];
    ...
    {
        if ($swaps < 0) {
            throw new InvalidArgumentException('The number of swaps must' . " be greater than or equal to 0, {$swaps} given.");
        }
        $this->a = $a;
        $this->swaps = $swaps;
    }
