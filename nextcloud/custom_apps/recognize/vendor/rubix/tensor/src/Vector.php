...
class Vector implements Tensor
{
    ...
    {
        if ($n < 1) {
            throw new InvalidArgumentException('Number of elements' . " must be greater than 0, {$n} given.");
        }
    ...
    public static function linspace(float $min, float $max, int $n) : self
    {
        ...
        if ($n < 2) {
            throw new InvalidArgumentException('Number of elements' . " must be greater than 1, {$n} given.");
        }
        $k = $n - 1;
     ...
     * @param bool $validate
     */
    public final function __construct(array $a, bool $validate = \true)
    {
        if ($a and $validate) {
            $a = \array_values($a);
            foreach ($a as &$valueA) {
    ...
    public function reshape(int $m, int $n) : Matrix
    {
        $p = $m * $n;
        if ($p !== $this->n) {
            throw new InvalidArgumentException("{$p} elements are" . " expected but vector has {$this->n}.");
        }
        $k = 0;
        $b = [];
        while (count($b) < $m) {
    ...
    {
        if ($this->n !== $b->size()) {
            throw new DimensionalityMismatch('Vector A expects' . " {$this->n} elements but vector B has {$b->size()}.");
        }
        $sigma = 0.0;
    ...
    public function convolve(Vector $b, int $stride = 1) : self
    {
        $n = $b->size();
        if ($n > $this->n) {
            throw new InvalidArgumentException('Vector B cannot be' . ' larger than Vector A.');
        }
        if ($stride < 1) {
    ...
    public function multiply($b)
    {
        ...
        throw new InvalidArgumentException('Cannot multiply' . ' vector to the given input.');
    }
    /**
    ...
    public function divide($b)
    {
        ...
        throw new InvalidArgumentException('Cannot divide' . ' vector by the given input.');
    }
    /**
    ...
    public function add($b)
    {
        ...
        throw new InvalidArgumentException('Cannot add' . ' vector to the given input.');
    }
    /**
    ...
    public function subtract($b)
    {
        ...
        throw new InvalidArgumentException('Cannot subtract' . ' vector from the given input.');
    }
    /**
    ...
    public function pow($b)
    {
        ...
        throw new InvalidArgumentException('Cannot raise' . ' vector to the power of the given input.');
    }
    /**
    ...
    public function mod($b)
    {
        ...
        throw new InvalidArgumentException('Cannot mod' . ' vector with the given input.');
    }
    /**
    ...
    public function equal($b)
    {
        ...
        throw new InvalidArgumentException('Cannot compare' . ' vector with the given input.');
    }
    /**
    ...
    {
        return static::ones($this->n)->divideVector($this);
    }
    /**
    ...
    {
        if ($q < 0.0 or $q > 1.0) {
            throw new InvalidArgumentException('Q must be between' . " 0 and 100, {$q} given.");
        }
        $a = $this->a;
        \sort($a);
        $x = $q * ($this->n - 1) + 1;
    ...
    public function variance($mean = null) : float
    {
        ...
        $ssd = $this->subtractScalar($mean)->square()->sum();
        return $ssd / $this->n;
    }
    /**
    ...
    {
        if ($this->n !== $b->n()) {
            throw new DimensionalityMismatch('Vector A expects' . " {$this->n} columns but Matrix B has {$b->n()}.");
        }
        $c = [];
    ...
    public function offsetGet($index)
    {
        ...
        throw new InvalidArgumentException('Element not found at the' . " given offset {$index}.");
    }
    /**
