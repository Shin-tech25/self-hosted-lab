...
namespace OCA\Recognize\Vendor\Tensor;
...
use OCA\Recognize\Vendor\Tensor\Exceptions\DimensionalityMismatch;
use OCA\Recognize\Vendor\Tensor\Exceptions\RuntimeException;
...
class Matrix implements Tensor
{
    ...
    {
        if ($n < 1) {
            throw new InvalidArgumentException('N must be' . " greater than 0, {$n} given.");
        }
        $a = [];
        for ($i = 0; $i < $n; ++$i) {
            $rowA = [];
     ...
     * @throws InvalidArgumentException
     */
    public function __construct(array $a, bool $validate = \true)
    {
        $m = count($a);
        $n = count(\current($a) ?: []);
        if ($a and $validate) {
            ...
            foreach ($a as $i => &$rowA) {
                if (count($rowA) !== $n) {
                    throw new InvalidArgumentException('The number of columns' . " must be equal for all rows, {$n} required but " . count($rowA) . " given at row offset {$i}.");
                }
    ...
    {
        $a = self::identity($this->m)->augmentLeft($this)->rref()->a()->asArray();
        $b = [];
        foreach ($a as $rowA) {
    ...
    {
        if ($this->n !== $b->m()) {
            throw new DimensionalityMismatch('Matrix A requires' . " {$this->n} rows but Matrix B has {$b->m()}.");
        }
    ...
    {
        if ($this->n !== $b->size()) {
            throw new DimensionalityMismatch('Matrix A requires' . " {$this->n} elements but Vector B has {$b->size()}.");
        }
    ...
    public function convolve(Matrix $b, int $stride = 1) : self
    {
        ...
        if ($m > $this->m or $n > $this->n) {
            throw new InvalidArgumentException('Matrix B cannot be' . ' larger than Matrix A.');
        }
        if ($stride < 1) {
            throw new InvalidArgumentException('Stride cannot be' . " less than 1, {$stride} given.");
        }
        $b = $b->asArray();
        $p = \intdiv($m, 2);
        $q = \intdiv($n, 2);
    ...
    public function multiply($b)
    {
        ...
        throw new InvalidArgumentException('Cannot multiply' . ' matrix by the given input.');
    }
    /**
    ...
    public function divide($b)
    {
        ...
        throw new InvalidArgumentException('Cannot divide' . ' matrix by the given input.');
    }
    /**
    ...
    public function add($b)
    {
        ...
        throw new InvalidArgumentException('Cannot add' . ' matrix to the given input.');
    }
    /**
    ...
    public function subtract($b)
    {
        ...
        throw new InvalidArgumentException('Cannot subtract' . ' the given input from matrix.');
    }
    /**
    ...
    public function pow($b)
    {
        ...
        throw new InvalidArgumentException('Cannot raise' . ' matrix to the given power.');
    }
    /**
    ...
    public function mod($b)
    {
        ...
        throw new InvalidArgumentException('Cannot mod' . ' matrix with the given input.');
    }
    /**
    ...
    public function equal($b)
    {
        ...
        throw new InvalidArgumentException('Cannot compare' . ' matrix to the given input.');
    }
    /**
    ...
    {
        return self::ones(...$this->shape())->divideMatrix($this);
    }
    /**
    ...
    public function variance($mean = null) : ColumnVector
    {
        if (isset($mean)) {
            ...
            if ($mean->m() !== $this->m) {
                throw new DimensionalityMismatch('Mean vector must' . " have {$this->m} rows, {$mean->m()} given.");
            }
        } else {
            $mean = $this->mean();
        }
        return $this->subtractColumnVector($mean)->square()->sum()->divide($this->m);
    }
    /**
    ...
    {
        if ($q < 0.0 or $q > 1.0) {
            throw new InvalidArgumentException('Q must be between' . " 0 and 1, {$q} given.");
        }
        $x = $q * ($this->n - 1) + 1;
        $xHat = (int) $x;
        $remainder = $x - $xHat;
    ...
    public function covariance(?ColumnVector $mean = null) : self
    {
        ...
        return $b->matmul($b->transpose())->divideScalar($this->m);
    }
    /**
    ...
    {
        if ($precision < 0) {
            throw new InvalidArgumentException('Decimal precision cannot' . " be less than 0, {$precision} given.");
        }
        $b = [];
        foreach ($this->a as $rowA) {
            $rowB = [];
    ...
    public function augmentAbove(Matrix $b) : self
    {
        if ($this->m > 0 and $b->n() !== $this->n) {
            throw new DimensionalityMismatch('Matrix A requires' . " {$this->n} columns but Matrix B has {$b->n()}.");
        }
    ...
    public function augmentLeft(Matrix $b) : self
    {
        if ($this->m > 0 and $b->m() !== $this->m()) {
            throw new DimensionalityMismatch('Matrix A requires' . " {$this->m} rows but Matrix B has {$b->m()}.");
        }
    ...
    public function multiplyMatrix(Matrix $b) : self
    {
        if ($b->shape() !== $this->shape()) {
            throw new DimensionalityMismatch("{$this->shapeString()} matrix" . " expected but {$b->shapeString()} given.");
        }
        $c = [];
    ...
    {
        if ($b->n() !== $this->n) {
            throw new DimensionalityMismatch('Matrix A expects' . " {$this->n} columns but Vector B has {$b->n()}.");
        }
        $b = $b->asArray();
        $c = [];
        foreach ($this->a as $rowA) {
    ...
    {
        if ($b->m() !== $this->m) {
            throw new DimensionalityMismatch('Matrix A expects' . " {$this->m} rows but Vector B has {$b->m()}.");
        }
        $c = [];
