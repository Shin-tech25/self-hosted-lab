...
use OCA\Recognize\Vendor\Amp\Parallel\Sync\ChannelException;
use OCA\Recognize\Vendor\Amp\Parallel\Sync\ChannelledSocket;
use OCA\Recognize\Vendor\Amp\Parallel\Sync\ExitResult;
use OCA\Recognize\Vendor\Amp\Parallel\Sync\SynchronizationError;
use OCA\Recognize\Vendor\Amp\Process\Process as BaseProcess;
...
final class Process implements Context
{
    ...
    public function __construct($script, string $cwd = null, array $env = [], string $binary = null)
    {
        ...
        $command = \implode(" ", [\escapeshellarg($binary), $this->formatOptions($options, $otherOpts), \escapeshellarg($scriptPath), $this->hub->getUri(), $script]);
